...
class CadastroService:
    ...
    def _registro_foi_reaberto(
        ...
        """
        Regras:
        1) Se foi concluído no desktop e o evento atual NÃO é posterior à conclusão,
           permanece realizado.
        2) Só é reabertura/nova entrada quando existe evidência de um evento mais novo.
        3) Se nunca foi concluído no desktop, status base pendente/presente continua pendente.
        """
        ...
        # Caso concluído no desktop:
        # - se o evento atual for mais novo que a conclusão => reabriu
        # - se não for mais novo => continua realizado
        if concluido_no_sistema or status_balgate_norm == "realizado":
        ...
        # Caso ainda não concluído no desktop
        if self._status_indica_pendente(status_base):
            return True
