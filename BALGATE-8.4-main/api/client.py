...
class FirebaseClient:
    ...
    def _criar_sessao_http(self):
        ...
        # aumentados para ambiente corporativo / rede mais lenta
        session.SetTimeouts(10000, 10000, 15000, 20000)
    ...
    def autenticar(self, email, senha):
        ...
        try:
            ...
            resposta = session.ResponseText or "{}"

            if status != 200:
                print(f"Falha HTTP ao autenticar no Firebase. Status={status} Resposta={resposta}")
                self.id_token = None
                return False
            ...
            if not self.id_token:
                print("Autenticação sem idToken. Resposta:", resp)
                return False
    ...
    # -------------------------------------------------------------
    def _http_get(self, url, tentativas=3):
        ultimo_erro = None
        ...
        for tentativa in range(1, tentativas + 1):
            try:
                ...
                ultimo_erro = e
                print(f"Erro GET (WinHTTP) tentativa {tentativa}/{tentativas}:", e)
 
        print("Erro GET (WinHTTP): todas as tentativas falharam.", ultimo_erro)
        return None
    ...
    def _http_patch(self, url, data_dict):
        try:
            ...
            print("[PATCH] STATUS =", session.Status)
            print("[PATCH] RESPONSE =", session.ResponseText)
        except Exception as e:
